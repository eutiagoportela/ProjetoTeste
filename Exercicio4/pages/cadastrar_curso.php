    ...
    if(count($erro) == 0) {
        ...
        if($deu_certo !== false) {
            ...
            if(!$inserido)
                ...
                $logManager1->log('info', 'Curso  - '.$titulo .' - Cadastrado por '.$_SESSION['usuario']." = ".$_SESSION['nome']);
                $logManager2 = new DataBaseLogManager();
                $logManager2->log('info', 'Curso - '.$titulo .' - Cadastrado por '.$_SESSION['usuario']." = ".$_SESSION['nome']);
